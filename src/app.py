...
reft = [120.0, 300.0, 600.0, 1200.0, 2400.0]
refp = []
filename = "None"
reffile = " None.txt"
p = []
...
def inputWindow():
    ...
    layout = [
        [
            sg.Text(
                "Welcome to Anaerobic analysis software of Charles Dauwe",
                font=("any 16"),
            )
        ],
        [sg.Text("All info on www.TyphoonCycling.org", font=("any 16"))],
        [
            sg.Text("Your Recovery Index", font=("any 16"), size=(18, 1)),
            sg.InputText(
                "0.8", key="iri", size=(5, 1), font=("any 16"), justification="r"
            ),
        ],
        [
            sg.Text("Calibration Power-Duration points", font=("any 16")),
            sg.FileBrowse(
                "Choose File",
                font=("any 16"),
                key="reffile",
        ...
        [
            sg.Text("Best  2 Minutes Power", font=("any 16"), size=(18, 1)),
            sg.InputText(
                "0", key="refp1", size=(5, 1), font=("any 16"), justification="r"
            ),
            sg.Text("         RT", font=("any 16"), size=(8, 1)),
            sg.InputText(
                "0", key="rtfield", size=(5, 1), font=("any 16"), justification="r"
            ),
            sg.Text("W", font="any 16"),
        ],
        [
            sg.Text("Best  5 Minutes Power", font=("any 16"), size=(18, 1)),
            sg.InputText(
                "0", key="refp2", size=(5, 1), font=("any 16"), justification="r"
            ),
            sg.Text("         MAP", font=("any 16"), size=(8, 1)),
            sg.InputText(
                "0", key="mapfield", size=(5, 1), font=("any 16"), justification="r"
            ),
            sg.Text("W", font="any 16"),
        ],
        [
            sg.Text("Best 10 Minutes Power", font=("any 16"), size=(18, 1)),
            sg.InputText(
                "0", key="refp3", size=(5, 1), font=("any 16"), justification="r"
            ),
            sg.Text("         SCP", font=("any 16"), size=(8, 1)),
            sg.InputText(
                "0", key="scpfield", size=(5, 1), font=("any 16"), justification="r"
            ),
            sg.Text("W", font="any 16"),
        ],
        [
            sg.Text("Best 20 Minutes Power", font=("any 16"), size=(18, 1)),
            sg.InputText(
                "0", key="refp4", size=(5, 1), font=("any 16"), justification="r"
            ),
            sg.Text("         W", font=("any 16"), size=(8, 1)),
            sg.InputText(
                "0", key="wfield", size=(5, 1), font=("any 16"), justification="r"
            ),
            sg.Text("kJ", font="any 16"),
        ],
        [
            sg.Text("Best 40 Minutes Power", font=("any 16"), size=(18, 1)),
            sg.InputText(
                "0", key="refp5", size=(5, 1), font=("any 16"), justification="r"
            ),
            sg.Text("   VO2max", font=("any 16"), size=(8, 1)),
            sg.InputText(
                "0", key="vo2maxfield", size=(5, 1), font=("any 16"), justification="r"
            ),
            sg.Text("ml/min/kg", font="any 14"),
        ],
        [
            sg.Text("Body Mass", font=("any 16")),
            sg.InputText(
                "0", key="bmass", size=(5, 1), font=("any 16"), justification="r"
            ),
            sg.Text("kg", font="any 16", size=(4, 1)),
        ],
        [
            sg.Text("Show ECP graph ?", font=("any 16")),
            sg.Checkbox("", key="ptflag", size=(5, 1), font=("any 16")),
        ],
        [
            [
                sg.Text("Choose a CSV or FIT Power File : ", font=("any 16")),
                sg.FileBrowse(
                    key="filename", font=("any 16"), file_types=(("All Files", "*.*"),)
                ),
            ]
        ],
        [
            sg.Text("ROI starts at ", font=("any 16")),
            sg.InputText(
                "0", key="start", size=(5, 1), font=("any 16"), justification="r"
            ),
            sg.Text("Ends at", font=("any 16"), justification="r"),
            sg.InputText(
                "0", key="end", size=(5, 1), font=("any 16"), justification="r"
            ),
        ],
        [
            sg.Text("Smooth factor = ", font=("any 16")),
            sg.InputText(
                "1", key="sm", size=(2, 1), font=("any 16"), justification="r"
            ),
        ],
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED:  # or event == 'Cancel':
            ...
            if exists(reffile):
                ...
                sg.popup(
                    "Choose a power calibration file",
                    font=("any 18"),
            ...
            if exists(filename) and againflag:
                ...
                sg.popup(
                    "Choose Power and/or Calibration File",
                    title=" ",
                    background_color="Grey",
                    font="any 18",
                )
...
def calcBalance(CP, W, RT, scp, slope, power, iri):
    ...
    for i in range(2, n):
        ...
        if p[i] < RT:
            ...
            tau[i] = max(
                wbal[i - 1] / ((RT - p[i]) * iri), 10
            )  # Correction for IRI and limit value of 60 s
            ...
            anaplus = min(
                (W - wref) * (1 - np.exp(-dt / tau[i])),
                max(CP - p[i] - 0.28 * bmass, 0),
            )
...
def showpowerzonebars(p):
    ...
    for i in range(len(p)):
        ...
        for j in range(0, len(binlimits) - 1):
            ...
            z[j] = (
                z[j] + dt * pp * (pp > binlimits[j]) * (pp <= binlimits[j + 1]) / 1000
            )  # Kilojouse
...
if __name__ == "__main__":
    ...
    while True:
        ...
        ax1.fill_between(
            xplot / 3600, wbalpro, nh, facecolor="lightgrey", interpolate=True
        )
        ...
        ax2.axhline(
            ...
            linestyle="--",
            linewidth=2,
            label="SCP: %d W" % round(scp),
        )

        ax2.axhline(
            y=RT, color="g", linestyle="--", linewidth=2, label="RT: %d W" % round(RT)
        )
